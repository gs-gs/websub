...
def _deregister_subscription(callback, topic):
    use_case = SubscriptionDeregisterUseCase()
    result = use_case.execute(callback, topic)
    if not result:
...
def _register_subscription(callback, topic, expiration):
    use_case = SubscriptionRegisterUseCase()
    result = use_case.execute(callback, topic, expiration)
    if result is None:
...
@consumes(VALID_REQUEST_CONTENT_TYPE)
def subscription_register():
    ...
    if form[MODE_ATTR_KEY] == MODE_ATTR_SUBSCRIBE_VALUE:
        return _register_subscription(form[CALLBACK_ATTR_KEY], form[TOPIC_ATTR_KEY], form[LEASE_SECONDS_ATTR_KEY])
    elif form[MODE_ATTR_KEY] == MODE_ATTR_UNSUBSCRIBE_VALUE:
        return _deregister_subscription(form[CALLBACK_ATTR_KEY], form[TOPIC_ATTR_KEY])
    else:
